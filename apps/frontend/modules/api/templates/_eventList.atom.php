...
<?php foreach ($events as $e): ?>
    <entry>
      ...
      <link href="<?php echo url_for('event/show?id=' . $e['id'], true) ?>" />
      <id><?php echo sha1($e['id']) ?></id>
      <updated><?php echo gmstrftime('%Y-%m-%dT%H:%M:%SZ', strtotime($e['updated_at'])) ?></updated>
      <summary type="xhtml">
        <div xmlns="http://www.w3.org/1999/xhtml">
          <p>
            <strong><?php echo __('Location') ?>:</strong> <?php if ($e['location_id'] > 0) { echo $e['recurringLocation']['name']; } else { echo $e['customLocation']; } ?><br />
